...
def train_namat(nodes, X_final, A_list, mask_list, logdeg_list, y_np, splits, cfg: Config, run_id: int):
    ...
    for fold, (train_sel, val_sel, test_sel) in enumerate(splits, start=1):
        ...
        for ep in range(cfg.MAX_EPOCHS):
            ...
            if not cfg.QUIET and (ep % 40 == 0 or ep == cfg.MAX_EPOCHS - 1):
                print(
...
def main_one(cfg: Config, cancer: str, run_id: int = 1):
    ...
    feat_omic = load_features(cfg.FEATURES_CSV, cfg.FEATURES_GENE_COL, verbose=not cfg.QUIET)
    ...
    if cfg.DO_INTERSECTION_CHECK:
        print_intersection_check(ppi_raw_all, feat_omic, pos, neg, verbose=not cfg.QUIET)

    nodes = compute_global_nodes(
        ppi_raw_all, feat_omic, cfg.UNIVERSE_CACHE_PATH, cfg.CACHE_UNIVERSE, verbose=not cfg.QUIET
    )
...
def parse_args():
    ...
    parser.add_argument("--verbose", action="store_true",
                         help="Show detailed debug/diagnostic logs (intersection check, per-PPI "
                              "degree stats, gene-id auto-detection, etc.). Off by default so a "
                              "fresh run prints only headers, per-fold final results, and the "
                              "overall summary.")
